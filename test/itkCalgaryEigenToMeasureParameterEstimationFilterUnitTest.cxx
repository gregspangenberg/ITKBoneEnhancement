...
{
template <typename T>
class itkCalgaryEigenToMeasureParameterEstimationFilterUnitTest : public ::testing::Test
{
public:
  /* Useful typedefs */
  ...
  using FilterType = typename itk::CalgaryEigenToMeasureParameterEstimationFilter<EigenImageType>;
  ...
  itkCalgaryEigenToMeasureParameterEstimationFilterUnitTest()
  {
    /* Instantiate filter */
  ...
  ~itkCalgaryEigenToMeasureParameterEstimationFilterUnitTest() override = default;
...
TYPED_TEST_SUITE(itkCalgaryEigenToMeasureParameterEstimationFilterUnitTest, TestingLabelTypes);

TYPED_TEST(itkCalgaryEigenToMeasureParameterEstimationFilterUnitTest, InitialParameters)
{
  EXPECT_DOUBLE_EQ(0.5, this->m_Filter->GetFrobeniusNormWeight());

  /* Calgary uses 2 parameters: alpha and c */
  this->m_Parameters = this->m_Filter->GetParameters();
  EXPECT_EQ(2u, this->m_Parameters.GetSize());
  EXPECT_DOUBLE_EQ(0.5, this->m_Parameters[0]);
  EXPECT_DOUBLE_EQ(1.0, this->m_Parameters[1]);
}

TYPED_TEST(itkCalgaryEigenToMeasureParameterEstimationFilterUnitTest, TestZerosImage)
{
...
TYPED_TEST(itkCalgaryEigenToMeasureParameterEstimationFilterUnitTest, TestOnesImage)
{
  ...
  EXPECT_EQ(2u, this->m_Parameters.GetSize());
  EXPECT_DOUBLE_EQ(0.5, this->m_Parameters[0]);
  EXPECT_NEAR(0.86602540378, this->m_Parameters[1], 1e-6); // sqrt(3) * 0.5
}

TYPED_TEST(itkCalgaryEigenToMeasureParameterEstimationFilterUnitTest, TestOnesImageWithDifferentWeight)
{
  ...
  EXPECT_EQ(2u, this->m_Parameters.GetSize());
  EXPECT_DOUBLE_EQ(0.5, this->m_Parameters[0]);
  EXPECT_NEAR(0.17320508075, this->m_Parameters[1], 1e-6); // sqrt(3) * 0.1
}

TYPED_TEST(itkCalgaryEigenToMeasureParameterEstimationFilterUnitTest, DISABLED_TestWithSpatialObject)
{
  ...
  EXPECT_EQ(2u, this->m_Parameters.GetSize());
  EXPECT_DOUBLE_EQ(0.5, this->m_Parameters[0]);
  EXPECT_NEAR(86.6025403784, this->m_Parameters[1], 1e-6); // sqrt(3*100^2) * 0.5
}
